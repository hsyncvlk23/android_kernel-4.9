...
>>>>>>> a494a71146a1 (ANDROID: binder: remove waitqueue when thread exits.)
};
...
/* If this thread used poll, make sure we remove the waitqueue from any epoll data structures holding it with POLLFREE. waitqueue_active() is safe to use here because we're holding */
	if ((thread->looper & BINDER_LOOPER_STATE_POLL) &&
